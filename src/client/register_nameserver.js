...
async function main() {
  ...
  const s = await getStore(connection, 'nameserver.json')
  ...
  if ( alreadyVoted ) {
    ...
    console.log("Dude, name ", data, " is already taken!!!!")
    process.exit(0)
  }
  ...
  console.log("Metadata account created at:",newAccountPubkey.toString(),"for target:",ourAccount.publicKey.toString())
  ...
  const accountDataLayout = BufferLayout.struct([
    BufferLayout.u32('name_count'),
  ]);

  const counts = accountDataLayout.decode(Buffer.from(accountInfo.data))

  console.log("Vote counts, candidate1:", counts.name_count)
